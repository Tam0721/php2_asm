        ...
        <div class= "row">
            <div class ="col-sm-5">
                <div style="text-align: center; border:1px solid #ccc;">
                    <img id="show" width="70%" src="./public/upload/{{$detail_product->img}}" alt="">
                </div>

                <!-- demo -->
                <div style="text-align: center; margin-top: 10px;">
                    <img src="./public/upload/{{$detail_product->img}}" width="15%" onclick="myFunction(this)">
                    @foreach ($listImages as $img) 
                        <img src="./public/upload/{{$img->img}}" width="15%" onclick="myFunction(this)">
                    @endforeach
                </div>
                <!-- end demo -->
                ...
                <div style="margin-top: 20px;">
                    <h4>Mô tả</h4>
                    <p style="margin-left: 20px;">{{$detail_product->mota}}</p>
                </div>
            </div>
            <div class ="col-sm-3">
                ...
                <!-- <p style="font-weight:bold; text-align:center; color:red;font-size:25px;">$  -->
                        @if ($detail_product->price_new > 0)
                            <p style="font-weight:bold; text-align:center; color:red;font-size:25px;">
                                {{number_format($detail_product->price_new, 0, '.', '.')}}.000 ₫</p>
                            <p style="text-align:center; color:black;font-size:15px; text-decoration: line-through;">
                                {{number_format($detail_product->price_old, 0, '.', '.')}}.000 ₫</p>
                        @else
                            <p style="font-weight:bold; text-align:center; color:red;font-size:25px;">
                                {{number_format($detail_product->price_old, 0, '.', '.')}}.000 ₫</p>
                        @endif
                        <form action="index.php?url=cartprocess" method="post">
                            ...
                            <div class="">
                                ...
                                <div class="amount-form" style="margin-left: 20px;">
                                    ...
                                    <input type="hidden" name="price" value="{{($detail_product->price_new > 0)? $detail_product->price_new:$detail_product->price_old}}">
                                    <input type="hidden" name="img" value="{{$detail_product->img}}">
                                    <input type="hidden" name="note" value="0">

                        <!-- <button class="btn-plus" id="plus" onclick="handlePlus()"><i class="fa-solid fa-plus"></i></button>  -->
                        

                        <input type="submit" name="addgiohang" value="{{($detail_product->trang_thai)? "Đặt hàng":"Hết hàng"}}" {{$detail_product->trang_thai? "":"disabled"}} class="btn btn-default border-0" style="margin:0px 0 15px 15px; width:120px; background: linear-gradient(131deg, rgba(255,117,0,1) 12%, rgba(255,184,0,1) 86%); color:#fff;">
                ...
                <div class="">
                    ...
                    <p style="margin-left: 20px;">{{$detail_product->ten_loai}}</p>
                </div>
                <div class="">
                    <h6 style="margin-top: 20px;">Loại: </h6>
                    <p style="margin-left: 20px;">Giày {{$detail_product->ten_loai}}</p>
                </div>
    ...
    <!--  -->
        ...
        <script>
            ...
            //     txt = $("input").val();
                $("#binhluan").load("view/comment.php", {idpro: {{$detail_product->id}}});
            // });
            });
